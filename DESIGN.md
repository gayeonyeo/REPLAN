...
핵심 배분 계산은 결정론적 규칙 엔진으로 구현한다. 생성형 AI는 범위 텍스트 구조화, 사용자 친화적 설명, 대안 전략 제안에만 사용하며 계획량의 최종 숫자는 검증 가능한 계산 결과를 따른다.
...
AI 출력은 스키마 검증을 통과해야 하며, 범위·날짜·수용량 계산은 서버 규칙 엔진이 재검증한다. AI 장애 시에도 핵심 캘린더와 재계획 기능은 동작해야 한다.
