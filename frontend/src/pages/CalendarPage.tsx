...
export default function CalendarPage() {
  ...
  const openDay = (date: string) => { setSelectedDate(date); setCursor(new Date(`${date}T00:00:00`)); setView('day') }
  const movePeriod = (amount: number) => { const next = new Date(cursor); if (view === 'month') next.setMonth(next.getMonth() + amount, 1); else if (view === 'week') next.setDate(next.getDate() + amount * 7); else next.setDate(next.getDate() + amount); setCursor(next); if (view === 'day') setSelectedDate(dateKey(next)) }
  const title = view === 'month' ? `${cursor.getFullYear()}년 ${cursor.getMonth() + 1}월` : view === 'week' ? `${dateKey(weekDays[0])} – ${dateKey(weekDays[6])}` : new Date(`${selectedDate}T00:00:00`).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'long' })
  ...
  return <Layout>
    <header className="calendar-header"><div><span className="kicker">INTEGRATED CALENDAR</span><h1>나의 시간표</h1></div><div className="calendar-actions"><div className="view-switch" role="group" aria-label="캘린더 보기"><button className={view === 'month' ? 'active' : ''} onClick={() => setView('month')}>월</button><button className={view === 'week' ? 'active' : ''} onClick={() => setView('week')}>주</button><button className={view === 'day' ? 'active' : ''} onClick={() => { setSelectedDate(dateKey(cursor)); setView('day') }}>일</button></div><button className="btn btn-primary" onClick={() => setShowForm(value => !value)}>+ 고정 일정</button></div></header>
    ...
    <section className="calendar-shell"><div className="calendar-toolbar"><button onClick={() => movePeriod(-1)}>‹</button><h2>{title}</h2><button onClick={() => movePeriod(1)}>›</button><button className="today-button" onClick={() => { const today = new Date(); setCursor(today); setSelectedDate(dateKey(today)) }}>오늘</button></div>
      {view === 'month' && <div className="month-layout"><div className="month-calendar"><div className="weekday-row">{WEEKDAYS.map(day => <span key={day}>{day}</span>)}</div><div className="month-grid">{monthDays.map(day => { const key = dateKey(day); const entries = items.filter(item => item.date === key); return <button className={`month-day ${day.getMonth() !== cursor.getMonth() ? 'outside' : ''} ${key === dateKey(new Date()) ? 'today' : ''}`} key={key} onClick={() => openDay(key)}><span className="day-number">{day.getDate()}</span><div className="month-events">{entries.slice(0, 3).map(item => <span className={`month-event ${item.tone}`} key={item.id}>{item.startTime} {item.title}</span>)}{entries.length > 3 && <small>+{entries.length - 3}</small>}</div></button> })}</div></div><aside className="exam-progress-panel"><span className="kicker">EXAM PROGRESS</span><h3>다가오는 시험</h3>{data.exams.map(exam => { const percent = Math.min(100, exam.current_passes / exam.target_passes * 100); return <article key={exam.id}><div><b>{exam.subject}</b><time>{exam.exam_date} {exam.exam_time}</time></div><p>{exam.current_passes} / {exam.target_passes}회독</p><div className="progress"><i style={{ width: `${percent}%` }}/></div><small>현재 속도 예상 {exam.forecast_passes}회독</small></article> })}{data.exams.length === 0 && <p className="panel-empty">등록된 시험이 없습니다.</p>}</aside></div>}
      ...
      {view === 'day' && <div className="day-calendar"><div className="day-tip"><b>블록 위를 잡아 이동</b><span>아래 손잡이를 드래그해 길이 조절 · 15분 단위 · 겹치면 저장되지 않아요</span></div><div className="timeline"><div className="time-axis">{HOURS.map(hour => <span key={hour}>{String(hour).padStart(2, '0')}:00</span>)}</div><div className="time-track">{HOURS.map(hour => <i className="hour-line" style={{ top: `${(hour - 6) * PIXELS_PER_HOUR}px` }} key={hour}/>)}{dayItems.map(renderBlock)}{dayItems.length === 0 && <div className="day-empty">등록된 일정이 없습니다.</div>}</div></div></div>}
    </section>
  </Layout>
}
