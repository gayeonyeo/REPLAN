...
export default function Dashboard() {
  ...
  const resetDemo = async () => { setLoading(true); try { setData(await api.resetDemo()); setMessage('시연 데이터가 준비되었습니다. 실제 공부량을 기록해 계획 변화를 확인해 보세요.'); setError('') } catch (reason) { setError((reason as Error).message) } finally { setLoading(false) } }
  ...
  return <Layout>
    <section className="hero replan-hero"><div><span className="kicker">KHUNIV · NEXUS CHALLENGE</span><h1>RE:PLAN</h1><p className="hero-tagline">학습 계획을 만들고 디벨롭해주는 나만의 캘린더</p><div className="hero-actions"><button className="btn btn-primary" onClick={resetDemo}>30초 데모</button><Link className="btn btn-secondary" to="/exams/new">시험 등록</Link></div></div><div className="replan-orbit"><b>계획</b><span>기록</span><strong>재분배</strong><small>할수록 더 나에게 맞는 계획</small></div></section>
