...
RE:PLAN은 대학생의 고정 일정, 시험 범위·시간·우선 챕터, 자유롭게 작성한 과목 특성·계획 요구사항, 목표 회독 수와 실제 공부 수행 기록을 함께 사용해 시험 계획을 만들고 계속 재조정하는 웹 서비스다. 한 번 만든 계획을 보여주는 데서 끝나지 않고 `계획 생성 → 수행 기록 → 차이 감지 → 전체 재계획`의 피드백 루프를 반복한다.
...
- 시험 삭제, 같은 날 여러 시험의 시간별 등록, 우선 챕터와 자연어 계획 요구사항 입력
- 월간 3 : 일간 2 비율의 한 화면 통합 캘린더(날짜 선택 시 오른쪽 타임테이블 갱신)
- 시작일과 종료일을 지정하는 매주 반복 일정 및 드래그로 조절 가능한 월간·일간 캘린더 비율
- OpenAI 계획 생성에 `reasoning.effort: minimal`을 적용하고 생성·재생성 중에만 전용 진행 상태 표시
- 저장된 체크인에서 완료율·계획 대비 수행 비율·최근 속도·강한 요일과 시간대를 계산해 GPT 계획에 반영
- GPT 재계획의 회독·범위 누락 또는 중복을 현재 진도부터 연속된 범위로 자동 보정
